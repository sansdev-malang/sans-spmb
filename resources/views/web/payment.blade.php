                        ...
                        <div class="space-y-2.5">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-2.5">
                                 @forelse($channels as $channel)
                                    <label class="border border-slate-200 dark:border-slate-800 hover:border-brand-emerald dark:hover:border-emerald-600 bg-white dark:bg-slate-900 has-[:checked]:border-emerald-800 has-[:checked]:ring-1 has-[:checked]:ring-brand-emerald has-[:checked]:bg-emerald-50/15 dark:has-[:checked]:bg-emerald-950/20 rounded-xl px-2.5 py-1.5 h-11 sm:h-12 flex items-center gap-2 cursor-pointer transition shadow-xs hover:shadow-sm relative select-none">
                                        <input type="radio" name="payment_method" value="{{ $channel->code }}" data-type="{{ $channel->type }}" data-gateway="{{ $channel->gateway->code ?? '' }}" data-fee-type="{{ $channel->fee_type ?? 'flat' }}" data-fee-value="{{ $channel->fee_value ?? 4500 }}" class="sr-only peer" {{ $loop->first ? 'checked' : '' }}>
