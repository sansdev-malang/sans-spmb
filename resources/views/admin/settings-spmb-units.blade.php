...
<div class="p-8">
    ...
    @if(session('success'))
        <div class="mb-6 bg-green-50 text-green-700 p-4 rounded-xl text-sm border border-green-200 font-semibold shadow-sm flex items-center gap-2">
    ...
    @if(session('error'))
        <div class="mb-6 bg-red-50 text-red-700 p-4 rounded-xl text-sm border border-red-200 font-semibold shadow-sm flex items-center gap-2">
                            ...
                            <tr class="hover:bg-slate-50/30 transition">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('admin.spmb-settings.units.delete', $unit->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Unit ini? Data yang terhapus tidak dapat dikembalikan.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 bg-slate-50 hover:bg-rose-50 rounded-lg transition" title="Hapus Unit" {{ $unit->registrations_count > 0 ? 'disabled' : '' }}>
                            ...
                            <tr class="hover:bg-slate-50/30 transition">
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form action="{{ route('admin.spmb-settings.grades.delete', $grade->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Tingkatan ini? Data yang terhapus tidak dapat dikembalikan.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 bg-slate-50 hover:bg-rose-50 rounded-lg transition" title="Hapus Tingkatan" {{ $grade->registrations_count > 0 ? 'disabled' : '' }}>
                                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                                            </button>
                                        </form>
                                    </div>
