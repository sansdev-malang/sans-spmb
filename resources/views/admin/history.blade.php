                ...
                <div class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    ...
                    @if(isset($periods) && $periods->isNotEmpty())
                        ...
                        <div x-data="{ open: false }" class="relative inline-block text-left" @click.outside="open = false">
                            <input type="hidden" name="period_id" id="filter_period_id_history" value="{{ $selectedPeriodId ?? 'all' }}">
                            ...
                            <div x-show="open" 
                                ...
                                <button type="button" 
                                        @click="document.getElementById('filter_period_id_history').value = 'all'; open = false; document.getElementById('historyFilterForm').submit();"
                                ...
                                @foreach($periods as $period)
                                    ...
                                    <button type="button" 
                                            @click="document.getElementById('filter_period_id_history').value = '{{ $period->id }}'; open = false; document.getElementById('historyFilterForm').submit();"
                ...
                feeCategories.forEach((cat, idx) => {
                    ...
                    cat.items.forEach(item => {
                        ...
                        if (item.status === 'dispensation' || item.is_dispensation) {
                            statusBadge = '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wide bg-purple-100 dark:bg-purple-950/70 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/60"><i data-lucide="award" class="w-3 h-3"></i> Bebas Biaya</span>';
                        ...
                        const displayAmount = isDisp ? `Rp 0 <span class="text-[10px] text-purple-600 dark:text-purple-400 font-semibold block sm:inline sm:ml-1">(Dibebaskan)</span>` : `Rp ${itemAmount.toLocaleString('id-ID')}`;
