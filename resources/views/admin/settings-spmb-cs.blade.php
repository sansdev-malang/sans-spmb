    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                ...
                @else
                    Kelola nomor WhatsApp dan nama petugas admin panitia khusus untuk unit {{ $units->first()?->name }}.
                @endif
            </p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.spmb-settings.cs.save') }}" method="POST" class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                @if($isSuperAdmin)
                    <!-- Card 1: Pusat Layanan CS Utama (Super Admin Only) -->
                        ...
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="font-extrabold text-slate-800 dark:text-white text-sm">
                                    @if($isSuperAdmin)
                                        Kontak Admin Khusus per Unit Sekolah
                                    @else
                                        Kontak Admin Panitia {{ $units->first()?->name }}
                                    @endif
                                </h3>
                ...
                <div class="pt-2">
                    <button type="submit" class="px-6 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-extrabold transition shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2">
                        ...
                        @if($isSuperAdmin)
                            Simpan Seluruh Pengaturan Customer Service
                        @else
                            Simpan Kontak Panitia {{ $units->first()?->name }}
                        @endif
                    </button>
                </div>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
                        <div class="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                            ...
                            <h4 class="font-extrabold text-slate-800 dark:text-white text-xs uppercase tracking-wider">
                                ...
                                @else
                                    Preview Kontak Panitia {{ $units->first()?->code }}
                                @endif
                            </h4>
                        </div>
                        ...
                        <p class="text-xs text-slate-500 leading-relaxed">
                            ...
                            @else
                                Berikut adalah kartu informasi kontak unit yang terhubung saat calon pendaftar mengklik bantuan untuk unit {{ $units->first()?->name }}:
                            @endif
                        </p>
                        ...
                        <div class="pt-2">
                            ...
                            @else
                                @php $myUnit = $units->first(); @endphp
                                <div class="bg-slate-50 dark:bg-slate-950 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 space-y-3">
                                    <div class="flex items-center gap-3">
                                        <div class="h-10 w-10 rounded-2xl bg-emerald-600 text-white flex items-center justify-center font-bold text-xs shadow-sm">
                                            <i data-lucide="message-circle" class="w-5 h-5"></i>
                                        </div>
                                        <div>
                                            <h5 class="font-extrabold text-xs text-slate-800 dark:text-white">{{ $myUnit?->admin_contact_name ?: ('Admin Panitia ' . $myUnit?->code) }}</h5>
                                            <p class="text-[11px] font-mono text-emerald-600 font-bold">{{ $myUnit?->whatsapp_number ?: 'Belum diisi' }}</p>
                                        </div>
                                    </div>
                                    <div class="text-[11px] text-slate-500 dark:text-slate-400 border-t border-slate-200/80 dark:border-slate-800 pt-2.5">
                                        Pendaftar dapat langsung menghubungi nomor ini saat memilih unit <strong>{{ $myUnit?->name }}</strong>.
                                    </div>
                                </div>
                            @endif
                        </div>
