...
<div class="space-y-6">
    ...
    @if(count($units) > 1)
        <div class="flex items-center gap-2 overflow-x-auto">
            @foreach($units as $u)
                ...
                @endphp
                <a href="{{ route('admin.taaruf', array_merge(request()->query(), ['unit_id' => $u->id, 'period_id' => $selectedPeriodId])) }}" 
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <a href="{{ route('admin.taaruf', array_merge(request()->query(), ['status' => 'all', 'period_id' => $selectedPeriodId])) }}" 
        ...
        <!-- 2. Belum Terjadwal -->
        <a href="{{ route('admin.taaruf', array_merge(request()->query(), ['status' => 'unscheduled', 'period_id' => $selectedPeriodId])) }}" 
        ...
        <a href="{{ route('admin.taaruf', array_merge(request()->query(), ['status' => 'scheduled', 'period_id' => $selectedPeriodId])) }}" 
        ...
        <a href="{{ route('admin.taaruf', array_merge(request()->query(), ['status' => 'has_result', 'period_id' => $selectedPeriodId])) }}" 
    ...
    <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-150 dark:border-slate-800 shadow-sm">
        <form method="GET" action="{{ route('admin.taaruf') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            @if($currentUnitId)
            ...
            @if(isset($periods) && $periods->isNotEmpty())
                <div>
                    <select name="period_id" onchange="this.form.submit()" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand-emerald">
                        <option value="all" {{ $selectedPeriodId === 'all' ? 'selected' : '' }}>Semua Tahun Ajaran</option>
                        @foreach($periods as $p)
                            <option value="{{ $p->id }}" {{ (string)$selectedPeriodId === (string)$p->id ? 'selected' : '' }}>
                                {{ $p->name ?? ('TA ' . $p->year) }}{{ $p->is_active ? ' (Aktif)' : '' }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request()->hasAny(['search', 'status', 'date', 'per_page', 'period_id']))
                    <a href="{{ route('admin.taaruf', ['unit_id' => $currentUnitId, 'period_id' => $selectedPeriodId]) }}" class="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold transition" title="Reset Filter">
                                    ...
                                    @if(!$isCompleted)
                                        @if($isScheduled)
                                            ...
                                            <button type="button" 
                                                    ...
                                                    data-is-scheduled="1"
                                                    class="h-8 px-2.5 bg-emerald-50 hover:bg-emerald-100 text-brand-emerald dark:bg-emerald-950/40 dark:hover:bg-emerald-900/60 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer"
                                                    title="Ubah Jadwal Ta'aruf">
                                                <i data-lucide="calendar-cog" class="w-3.5 h-3.5"></i>
                                                <span>Edit</span>
                                            </button>

                                            <!-- Delete/Cancel Schedule (Icon button) -->
                                            <button type="button" 
                                                onclick="showConfirmDialog({
                                                    title: 'Batalkan Jadwal Ta\'aruf',
                                                    message: 'Batalkan jadwal Ta\'aruf ananda {{ addslashes($reg->candidate_name) }}? Jadwal observasi akan dihapus dan calon murid dikembalikan ke antrean belum terjadwal.',
                                                    confirmText: 'Ya, Batalkan Jadwal',
                                                    type: 'danger',
                                                    icon: 'calendar-x',
                                                    formAction: '{{ route('admin.taaruf.schedule.delete', $reg->id) }}',
                                                    formMethod: 'DELETE'
                                                })"
                                                class="h-8 w-8 flex items-center justify-center bg-slate-50 hover:bg-rose-50 text-slate-400 hover:text-rose-600 dark:bg-slate-800/80 dark:hover:bg-rose-950/50 dark:text-slate-400 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-700 rounded-xl text-xs transition cursor-pointer" 
                                                title="Batalkan Jadwal Ta'aruf">
                                        ...
                                        @else
                                            ...
                                            <button type="button" 
                                                    ...
                                                    data-unit-code="{{ $reg->unit?->code }}"
                                                    data-grade-name="{{ $reg->grade?->name }}"
                                                    data-observation-date="{{ date('Y-m-d') }}"
                                                    data-observation-time="Sesi 1 (08:00 - 09:30 WIB)"
                                                    data-observation-location="{{ $defaultLocation }}"
                                                    data-observation-room="{{ $defaultRoom }}"
                                                    data-observation-address="{{ $defaultAddress }}"
                                                    data-observation-interviewer=""
                                                    data-observation-notes=""
                                                    data-is-scheduled="0"
                                                    class="h-8 px-3 bg-brand-emerald hover-emerald text-white rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-2xs cursor-pointer"
                                                    title="Atur Jadwal Ta'aruf">
                                    ...
                                    @else
                                        ...
                                        <div class="inline-flex items-center gap-1.5">
                                            ...
                                            @if($reg->registration_status === 'taaruf_completed')
                                                <button type="button" 
                                                    onclick="showConfirmDialog({
                                                        title: 'Batalkan Penyelesaian Ta\'aruf',
                                                        message: 'Batalkan penyelesaian Ta\'aruf ananda {{ addslashes($reg->candidate_name) }} dan kembalikan ke tahap penjadwalan / observasi?',
                                                        confirmText: 'Ya, Kembalikan Status',
                                                        type: 'warning',
                                                        icon: 'rotate-ccw',
                                                        formAction: '{{ route('admin.taaruf.revert', $reg->id) }}',
                                                        formMethod: 'POST'
                                                    })"
                                                    class="h-8 px-2.5 bg-amber-50 hover:bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:hover:bg-amber-900/60 dark:text-amber-400 border border-amber-200 dark:border-amber-800 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-2xs cursor-pointer" 
                                                    title="Batal Selesaikan / Kembalikan ke Jadwal">
                                                    <i data-lucide="rotate-ccw" class="w-3.5 h-3.5"></i>
