                        ...
                        <tr id="cand-row-{{ $cand->id }}" class="group transition-colors duration-150 hover:bg-emerald-50/20 dark:hover:bg-slate-800/50">
                            ...
                            <td class="border-b border-slate-200 dark:border-slate-700 px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    ...
                                    @endphp
                                    <!-- Button Dispensasi Penerimaan -->
                                    <button type="button" 
                                            onclick='openDispensationModal(@json($dispPayload))'
                                            class="p-2 rounded-xl {{ $cand->is_dispensation ? 'bg-purple-100 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-700' : 'bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 hover:bg-purple-600 hover:text-white dark:hover:bg-purple-600 border border-purple-200 dark:border-purple-800/60' }} transition shadow-2xs"
                                            title="{{ $cand->is_dispensation ? 'Diterima melalui Dispensasi (' . $cand->dispensation_reason . ')' : 'Dispensasi Penerimaan (Nyatakan Diterima)' }}">
...
<!-- ========================================================================= -->
<!-- MODAL 3: DISPENSASI PENERIMAAN MANUAL SISWA                               -->
<!-- ========================================================================= -->
<div id="dispensation-modal" onclick="if(event.target === this) closeDispensationModal()" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="bg-gradient-to-r from-purple-800 to-indigo-900 p-6 text-white flex items-center justify-between">
            <div class="flex items-center gap-3.5">
                ...
                <div>
                    <h3 id="disp-cand-name" class="text-base font-extrabold text-white"></h3>
                    <div class="flex items-center gap-2 mt-0.5 text-xs text-purple-200">
                        <span id="disp-cand-id" class="font-mono font-bold select-all"></span>
                        <span>•</span>
                        <span id="disp-cand-unit"></span>
                    </div>
                </div>
            </div>
            <button type="button" onclick="closeDispensationModal()" class="h-9 w-9 rounded-xl bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition">
                <i data-lucide="x" class="w-5 h-5"></i>
            </button>
        </div>

        <!-- Body -->
        <form id="dispensation-form" onsubmit="submitDispensationForm(event); return false;" class="p-6 space-y-5">
            @csrf
            <input type="hidden" id="disp_registration_id" name="registration_id" value="">
            ...
            <div class="p-4 rounded-2xl bg-purple-50/70 dark:bg-purple-950/40 border border-purple-150 dark:border-purple-800/60 space-y-2">
                <div class="flex items-center justify-between text-xs font-semibold">
                    <span class="text-slate-500 dark:text-slate-400">Total Biaya Bersih (Netto):</span>
                    <span id="disp_net_fee" class="font-mono font-bold text-slate-800 dark:text-slate-200">Rp 0</span>
                </div>
                <div class="flex items-center justify-between text-xs font-semibold">
                    <span class="text-emerald-600 dark:text-emerald-400">Telah Terbayar:</span>
                    <span id="disp_total_paid" class="font-mono font-bold text-emerald-600 dark:text-emerald-400">Rp 0</span>
                </div>
                <div class="flex items-center justify-between text-xs font-bold pt-1.5 border-t border-purple-200/60 dark:border-purple-800">
                    ...
                    <span id="disp_remaining" class="font-mono text-sm font-extrabold text-amber-700 dark:text-amber-400">Rp 0</span>
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select id="disp_reason" name="reason" required class="w-full px-3.5 py-2.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-purple-500">
            ...
            <div class="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 flex items-start gap-2.5 text-xs text-amber-900 dark:text-amber-300">
                ...
                <p class="leading-relaxed">
                    Dengan menetapkan dispensasi ini, calon siswa akan <strong>langsung resmi berstatus DITERIMA (Completed)</strong> di sistem dan portal orang tua, meskipun sisa tagihan di atas belum lunas. Sisa tagihan tetap tercatat sebagai piutang berjalan.
                </p>
            </div>

            <!-- Modal Footer Buttons -->
            <div class="flex items-center justify-between gap-3 pt-2">
                <button type="button" id="btn_revert_dispensation" onclick="revertDispensation()" class="hidden px-4 py-2.5 bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 text-rose-700 dark:text-rose-300 rounded-xl text-xs font-bold transition flex items-center gap-1.5 border border-rose-200 dark:border-rose-800">
                    <i data-lucide="rotate-ccw" class="w-4 h-4"></i> Batalkan Dispensasi
                </button>
                <div class="flex items-center gap-2 ml-auto">
                    <button type="button" onclick="closeDispensationModal()" class="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 rounded-xl text-xs font-bold transition">
                        Tutup
                    </button>
                    <button type="submit" id="btn_save_dispensation" class="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold transition shadow-sm flex items-center gap-1.5">
...
<script>
    ...
    let currentDispCand = null;

    function openDispensationModal(cand) {
        document.body.classList.add('overflow-hidden');
        currentDispCand = cand;

        document.getElementById('disp-cand-name').textContent = cand.candidate_name || 'Calon Siswa';
        document.getElementById('disp-cand-id').textContent = cand.id_label || ('SANS-2027-' + String(cand.id).padStart(4, '0'));
        document.getElementById('disp-cand-unit').textContent = (cand.unit_name || 'Unit') + (cand.admission_level ? ' (' + cand.admission_level + ')' : '');
        document.getElementById('disp_registration_id').value = cand.id;

        document.getElementById('disp_net_fee').textContent = 'Rp ' + Number(cand.net_fee || 0).toLocaleString('id-ID');
        document.getElementById('disp_total_paid').textContent = 'Rp ' + Number(cand.total_paid || 0).toLocaleString('id-ID');
        document.getElementById('disp_remaining').textContent = 'Rp ' + Number(cand.remaining_balance || 0).toLocaleString('id-ID');

        const revertBtn = document.getElementById('btn_revert_dispensation');
        if (cand.is_dispensation) {
        ...
        const reasonSelect = document.getElementById('disp_reason');
        if (cand.dispensation_reason) {
        ...
        document.getElementById('dispensation-modal').classList.remove('hidden');
        if (window.lucide) lucide.createIcons();
    }

    function closeDispensationModal() {
        document.body.classList.remove('overflow-hidden');
        document.getElementById('dispensation-modal').classList.add('hidden');
    }

    async function submitDispensationForm(event) {
        if (event) {
            event.preventDefault();
            event.stopPropagation();
        }
        const regId = document.getElementById('disp_registration_id').value;
        const reason = document.getElementById('disp_reason').value;
        const saveBtn = document.getElementById('btn_save_dispensation');
    ...
    async function revertDispensation() {
        if (!currentDispCand || !confirm('Apakah Anda yakin ingin membatalkan status dispensasi calon siswa ini?')) {
            return;
        }

        const regId = currentDispCand.id;
        const revertBtn = document.getElementById('btn_revert_dispensation');
