...
@section('title', 'Peninjauan Pendaftaran - Portal SPMB')
@section('page_title', 'Peninjauan Pendaftaran')
...
<div class="space-y-8">
    ...
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            <h2 class="text-2xl font-extrabold leading-7 text-slate-900 sm:text-3xl sm:truncate">
        ...
        <div class="mt-4 flex md:mt-0 md:ml-4 gap-3">
            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center px-4 py-2 border border-slate-300 rounded-xl shadow-sm text-xs font-bold text-slate-700 bg-white hover:bg-slate-50 transition">
    ...
    @if (session('success'))
        <div class="mb-6 bg-green-50 text-green-700 p-4 rounded-xl text-sm border border-green-200 font-semibold shadow-sm animate-pulse">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <!-- Stat Item -->
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-yellow-500">
        ...
        <!-- Stat Item -->
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-green-500">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 border-l-4 border-l-emerald-500">
            ...
            <span class="text-2xl font-black text-brand-emerald block mt-1">{{ $stats['paid'] }}</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden">
        <div class="bg-slate-50 border-b border-slate-100 px-6 py-4 flex justify-between items-center">
            ...
            <div class="flex gap-2 text-[10px] font-bold">
                <a href="{{ route('admin.peninjauan') }}" class="px-2.5 py-1 rounded-full {{ !request()->has('status') ? 'bg-brand-emerald text-white' : 'bg-slate-200 text-slate-600' }}">Semua</a>
                <a href="{{ route('admin.peninjauan', ['status' => 'submitted']) }}" class="px-2.5 py-1 rounded-full {{ request()->status === 'submitted' ? 'bg-brand-emerald text-white' : 'bg-slate-200 text-slate-600' }}">Perlu Review</a>
                <a href="{{ route('admin.peninjauan', ['status' => 'verified']) }}" class="px-2.5 py-1 rounded-full {{ request()->status === 'verified' ? 'bg-brand-emerald text-white' : 'bg-slate-200 text-slate-600' }}">Terverifikasi</a>
                <a href="{{ route('admin.peninjauan', ['status' => 'failed']) }}" class="px-2.5 py-1 rounded-full {{ request()->status === 'failed' ? 'bg-brand-emerald text-white' : 'bg-slate-200 text-slate-600' }}">Ditolak</a>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-100 text-[10px] text-slate-400 font-bold uppercase tracking-wider bg-slate-50/50">
                        ...
                        <th class="py-4 px-6 text-right">Aksi Peninjauan</th>
                    </tr>
                </thead>
                <tbody class="text-sm divide-y divide-slate-100">
                    @forelse($registrations as $reg)
                        @php
                            $candJson = [
                                'id_label' => 'SANS-2026-' . str_pad($reg->id, 4, '0', STR_PAD_LEFT),
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="py-4 px-6 font-mono text-xs text-slate-500">
                                SANS-2026-{{ str_pad($reg->id, 4, '0', STR_PAD_LEFT) }}
                            </td>
                            <td class="py-4 px-6">
                                <div class="font-bold text-slate-800">{{ $reg->candidate_name ?? 'Draft' }}</div>
                            ...
                            <td class="py-4 px-6 text-center">
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider
                                    @if($reg->registration_status === 'verified') bg-green-50 text-green-700 border border-green-200
                                ...
                                <div class="flex justify-end gap-1.5">
                                    ...
                                    @if ($reg->registration_status === 'submitted')
                                        ...
                                        <form action="{{ route('admin.registrations.verify', $reg->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="bg-green-600 hover:bg-green-700 text-white font-bold text-xs px-3 py-1.5 rounded-lg shadow-sm transition">
                                                Setujui
                                            </button>
                                        </form>

                                        <!-- Reject Trigger (shows custom reason input dialog) -->
                                        <button onclick="toggleRejectModal({{ $reg->id }}, '{{ $reg->candidate_name }}')" class="bg-red-600 hover:bg-red-700 text-white font-bold text-xs px-3 py-1.5 rounded-lg shadow-sm transition">
                                            Tolak
                                        </button>
                                    @endif
                                </div>
    ...
    <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-slate-100 overflow-hidden">
        ...
        <form id="rejectForm" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <span id="rejectCandidateName" class="font-extrabold text-slate-800 text-sm"></span>
            </div>
            <div>
                <label for="reason" class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Alasan Penolakan*</label>
                <textarea id="reason" name="reason" required rows="4"
                    class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm"
                    placeholder="Contoh: Berkas Akte Kelahiran buram dan tidak terbaca. Harap unggah ulang berkas yang lebih jelas."></textarea>
            </div>
            <div class="flex justify-end gap-2 pt-4">
                <button type="button" onclick="closeRejectModal()" class="border border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-xl text-xs font-bold transition">
                    Kembali
                </button>
        ...
        <div class="p-6 space-y-6 overflow-y-auto flex-grow text-xs text-slate-700">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 rounded-xl bg-slate-50 border border-slate-150">
                <div>
                    ...
                    <span id="det-period" class="font-bold text-slate-700">-</span>
                </div>
                <div>
                    <span class="text-[9px] font-bold text-slate-400 uppercase block">Gelombang</span>
                    <span id="det-wave" class="font-bold text-slate-700">-</span>
                </div>
                <div>
                    <span class="text-[9px] font-bold text-slate-400 uppercase block">Jalur Masuk</span>
                    <span id="det-type" class="font-bold text-slate-700">-</span>
                </div>
                <div>
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <span id="det-level" class="font-bold text-slate-800">-</span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <span id="det-father" class="font-semibold text-slate-800">-</span>
                    </div>
                    <div>
                        <span class="text-[9px] font-bold text-slate-400 uppercase block">Nama Ibu Kandung</span>
                        <span id="det-mother" class="font-semibold text-slate-800">-</span>
                    </div>
                    <div>
                        <span class="text-[9px] font-bold text-slate-400 uppercase block">No. HP Wali (WhatsApp)</span>
                        <span id="det-phone" class="font-mono text-slate-800">-</span>
                    </div>
                </div>
            </div>
